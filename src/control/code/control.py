...
positionTopic = "/vrpn_client_node/ARAV/pose"
...
class Listener:
	...
	def poseCallback (self, msg):
		...
		self.realValueX = msg.pose.position.x
		self.realValueY = msg.pose.position.y
		
		orientObj = msg.pose.orientation
...
poseSub = rospy.Subscriber (positionTopic, PoseStamped, listener.poseCallback, queue_size=1)
...
while not rospy.is_shutdown() and activation:
	...
	deltaX, deltaY, status = selectNextWP (waypointList, realValueX, realValueY, debug = True)
